...
export function initISSPosition () {
  return function (dispatch) {
    ...
    return fetch(`https://api.abhijangir.me/open-notify/iss-now`).then((response) => {
...
export function whoIsInSpace () {
  return function (dispatch) {
    return fetch(`https://api.abhijangir.me/open-notify/astros`).then((response) => {
      response.json().then(json => {
...
export function whenWillPassThroughYou (c) {
  return function (dispatch) {
    return fetch(`https://api.abhijangir.me/open-notify/pass?lat=${c.lat}&lon=${c.long}`, { mode: 'cors' })
      .then((response) => {
        response.json().then(json => {
